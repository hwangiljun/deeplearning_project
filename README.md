...
2025 시즌 712,516구에서 측정한 **ECE 0.0084**, 검증셋에서 적합한
**temperature 1.070**. T 가 1 에 가깝다는 것은 모델이 이미 보정되어 있어
별도 보정이 필요 없다는 뜻이다(적용해도 log-loss 1.1259 → 1.1248 로 거의
변화가 없다).

초기 앱이 `temperature = 5.0` 을 손으로 박아 확률을 균등에 가깝게 뭉갠 것은,
`WeightedRandomSampler` 와 Focal Loss 이중 보정으로 왜곡된 확률을 가리려던
대증요법이었다. 원인을 제거하니 보정 자체가 불필요해졌다.

### 클래스별 성능과 설계상의 맞바꿈

| 클래스 | 표본 | 재현율 | F1 | AUC |
|---|---|---|---|---|
| ball | 255,753 | 0.877 | 0.830 | 0.951 |
| called_strike | 115,670 | 0.704 | 0.601 | 0.900 |
| foul | 135,954 | 0.510 | 0.448 | 0.789 |
| field_out | 83,352 | 0.239 | 0.280 | 0.798 |
| swinging_strike | 78,324 | 0.182 | 0.242 | 0.777 |
| single | 27,512 | 0.000 | 0.000 | 0.785 |
| double | 7,745 | 0.000 | 0.000 | 0.791 |
| home_run | 5,650 | 0.000 | 0.000 | 0.835 |
| triple | 628 | 0.000 | 0.000 | 0.791 |
| hit_by_pitch | 1,928 | 0.146 | 0.232 | 0.994 |

안타 계열의 **재현율이 0 이다.** 사전 확률이 1~4% 인 클래스는 argmax 에서
한 번도 1위가 되지 못하기 때문이다. macro-F1 이 0.2633 으로 낮은 것도 대부분
이 때문이다.

그러나 같은 클래스들의 **AUC 는 0.785~0.835 로, 순위 매기기는 제대로 하고
있다.** 이는 의도한 맞바꿈이다. 재현율을 올리려면 오버샘플링이나 클래스
가중을 넣어야 하는데, 그러면 출력 확률이 실제 분포를 벗어나 캘리브레이션이
깨진다(초기 버전이 정확히 그 상태였다). 추천 엔진은 argmax 가 아니라
**확률 분포 전체를 기대 실점으로 적분**하므로, 여기서는 정직한 확률이
높은 재현율보다 중요하다.
...
수록된 산출물은 **2024·2025 정규시즌 전체 1,424,426구**로 만들었다.
분석 가능 선수는 타자 1,024명 / 투수 1,193명, 타자 표본 중앙값 1,286구다.
